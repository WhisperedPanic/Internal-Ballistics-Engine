...
function calculateDerivatives_SI(y, params) {
  ...
  // SMOOTH IGNITION MODEL - decays gradually instead of abrupt cutoff
  const P_ignition_Pa = 4e6;
  const ignitionDecay = Math.exp(-Z_clamped * 100);  // Smooth decay from 1 to 0
  const P_effective_Pa = P_base_Pa + P_ignition_Pa * ignitionDecay;
